...
**Domain:** `Media / Transcription` · **Suite:** `Media Creator Suite` · **Accent:** `#fb7185`
...
**Gemini Audio Transcriber** is a standalone, browser-based AI workflow app for **Media / Transcription**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.

**Core job:** Audio/video → structured transcript.
...
| `video`<br>video | text | Yes | — | Affects input: video. |
...
| `model`<br>model | select | No | — | Affects customization: model. |
| `speaker_count`<br>speaker count | text | No | — | Affects customization: speaker count. |
| `timestamp_granularity`<br>timestamp granularity | text | No | — | Affects customization: timestamp granularity. |
| `glossary`<br>glossary | text | No | — | Affects customization: glossary. |
| `privacy`<br>privacy | text | No | — | Affects customization: privacy. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9124
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9124`. |
...
- Export download
- Job history
- Gemini/Whisper/local STT required for raw audio
...
```bash
curl http://127.0.0.1:9124/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9124/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9124 uvicorn app.main:app --host 0.0.0.0 --port 9124
```
...
- `plugins/gemini-audio-transcriber.json` for plugin metadata.
